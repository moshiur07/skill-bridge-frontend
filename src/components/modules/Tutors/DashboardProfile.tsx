...
const profileSchema = z.object({
  ...
  hourly_rate: z
    ...
    .min(1, "Rate must be at least $1.")
    .max(1000, "Rate cannot exceed $1000."),
  // image can be a File (upload) or existing URL string or null
...
export default function TutorProfileClient({
  ...
  // local preview URL state for image (either profile.image or blob URL for selected file)
  ...
  const form = useForm({
    defaultValues: {
      bio: profile.bio ?? "",
      hourly_rate: profile.hourly_rate ?? 0,
      // image may be existing URL (string) or null
    ...
    onSubmit: async ({ value }) => {
      ...
      //   formData.append("hourly_rate", String(value.hourly_rate));

      //   // image field can be File (uploaded) or string URL (existing) or null
      ...
      //     value.image.trim() !== ""
      //   ) {
      //     // backend may expect an image_url field when passing URL instead of file
      //     formData.append("image_url", value.image.trim());
      //   } else {
      //     // explicit empty
      //     formData.append("image", "");
      //   }

      //   // categories as JSON (array of numbers)
      //   formData.append("categories", JSON.stringify(value.categories));

      //   const res = await fetch(`${API_BASE}/api/tutors/${profile.id}`, {
      //     method: "PUT",
      //     credentials: "include",
      //     // when sending FormData, do not set Content-Type header (browser will set multipart boundary)
      //     body: formData,
      //   });
  ...
  function validateField<K extends keyof ProfileFormValues>(
    ...
    // skip strict validation for image (can be File or URL)
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        field.handleChange(file as string | null);
                        // // manage preview URL
                        // if (file) {
                        ...
                        // } else {
                        //   // reset to existing profile URL if any
                        //   if (prevBlobRef.current) {
                        //     try {
                        //       URL.revokeObjectURL(prevBlobRef.current);
                        //     } catch {}
